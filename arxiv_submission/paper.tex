...
In most current information retrieval systems, and by extension in Retrieval-Augmented Generation, the query embedding is treated as a fixed object. A user's intent is encoded into a vector, and the system returns whatever lies nearest to that vector in the embedding space. This process is largely \emph{passive}: the system does not reason about what the user might want beyond what the query literally states. It cannot, for instance, find the deep learning equivalent of a classical machine learning algorithm, nor can it exclude a semantic concept without resorting to keyword filtering.

We hypothesize that this limitation may not be inherent to the embedding space itself, but rather to how retrieval systems currently \emph{use} it. Modern sentence transformers produce embeddings that encode rich semantic structure --- structure that supports algebraic manipulation. The celebrated word2vec analogy \emph{king $-$ man $+$ woman $=$ queen} \cite{mikolov2013} demonstrated that vector arithmetic preserves semantic relationships at the word level. More recently, RISE \cite{rise2025} showed that semantic-syntactic transformations correspond to rotational operations in embedding space across seven languages. Yet to our knowledge, no prior work has systematically validated whether such operations generalize to \emph{document-level} embeddings for practical retrieval.

This paper proposes and empirically evaluates what we term the \emph{algebraic retrieval} approach, in which the query embedding is explicitly transformed before retrieval. We unify this under a single formulation:
...
\begin{enumerate}
\item \textbf{Isotropy as a governing variable.} A cross-model analysis across six architectures suggests that embedding isotropy --- the degree to which embeddings cluster in the space --- is a strong predictor of whether algebraic operations help or hurt retrieval. We conjecture that this finding may generalize beyond our specific operations to other applications that manipulate sentence embeddings (steering vectors, concept erasure, representation engineering), though this remains to be tested.

\item \textbf{A broad empirical comparison of embedding-space operations for retrieval.} Seventeen candidate operations, spanning Euclidean geometry, hyperbolic geometry (M\"{o}bius addition, gyroscalar multiplication), optimal transport (Wasserstein barycenter), Clifford algebra (geometric rotors), matrix-based approaches (Boolean conceptors, Kronecker decomposition), and memory-inspired methods (hippocampal pattern completion), evaluated on BEIR benchmarks across six models. In our experiments, only Euclidean operations on normalized embeddings proved effective. We report the negative results for the remaining twelve operations in detail, as they may inform future work in this area.

\item \textbf{Controllable cross-domain result set modification.} Embedding rotation provides a continuous, deterministic, zero-inference-cost mechanism for shifting retrieval results toward a different domain. We test this on drug repurposing and legal analogy scenarios, observing structural displacement in the expected direction. We emphasize that this is a \emph{structural} observation --- the documents change in the expected direction --- not a \emph{utility} finding. Whether the surfaced documents constitute genuinely useful cross-domain analogies is an open empirical question that requires domain-expert evaluation, which we do not provide.
...
At the sentence and document level, vector arithmetic has received less systematic attention. RISE \cite{rise2025} demonstrated that semantic-syntactic transformations in multilingual sentence embeddings correspond to rotational operations, validating geometric structure across seven languages. VL-JEPA (Meta, 2025) showed compositional retrieval in multimodal embedding spaces. However, to our knowledge, none of these works validated document-level algebraic operations specifically for retrieval tasks.
...
To our knowledge, \emph{none} of these systems perform explicit algebraic transformations on query embeddings. They improve \emph{when}, \emph{how often}, and \emph{in what order} to retrieve, while the retrieval mechanism itself remains nearest-neighbor similarity on an unmodified query vector. Our work explores a complementary direction: algebraic retrieval could in principle be composed with any RAG variant.
...
Our reading of the literature suggests an underexplored intersection between three areas: the algebraic structure of embedding spaces (established at the word level, less explored at the document level), the RAG retrieval mechanism (which typically treats query embeddings as immutable), and embedding manipulation techniques (which operate on model internals rather than retrieval queries). We position algebraic retrieval at this intersection: it attempts to leverage the algebraic structure of document embeddings to manipulate retrieval queries, exploring capabilities that neither standard RAG nor embedding manipulation alone currently address.
...
\textbf{Comparison with HyDE.} We additionally evaluated Hypothetical Document Embeddings (HyDE) \cite{gao2023hyde} on SciFact (50 queries) using a local LLM (Gemma-3 12B) to generate hypothetical documents. HyDE achieved nDCG@10 $= 0.752$, a $-6.3\%$ degradation from the baseline --- \emph{worse} than both PRF ($-0.4\%$) and rotation at $\alpha = 0.3$ ($-6.1\%$). The hypothetical documents generated by the LLM shift the query embedding away from the relevant documents, producing an uncontrolled domain drift. This result illustrates a potential advantage of algebraic rotation: it provides a similar kind of embedding displacement to HyDE but with deterministic control via $\alpha$, at zero inference cost. A more comprehensive comparison with HyDE across multiple datasets would be needed to confirm this.
...
This trade-off illustrates what we see as the potential value of algebraic retrieval: it allows a user to continuously interpolate between ``retrieve what matches my query'' and ``discover what corresponds to my query in a different domain.'' To our knowledge, no existing retrieval method provides this particular combination of continuous, deterministic, zero-inference-cost control over domain shift.
...
\emph{The mechanism is straightforward.} Rotation requires only: (1) encoding a natural-language description of the target domain, (2) computing a weighted average of two vectors, and (3) normalizing. No additional training, no auxiliary models, no domain-specific knowledge graphs.

\emph{Rotation does not improve standard retrieval.} To be clear: algebraic rotation is not a replacement for existing retrieval methods. On all four BEIR datasets, the baseline outperforms rotation on nDCG@10 (Table~\ref{tab:beir_ndcg}). The value of rotation lies in enabling a \emph{different kind} of retrieval --- cross-domain analogical discovery --- that standard methods cannot provide.
...
The full inventory with empirical results is in Appendix~\ref{app:designspace}. In our experiments, \emph{only Euclidean operations on normalized embeddings proved effective for retrieval in cosine-similarity spaces}. Hyperbolic, optimal-transport, and matrix-based alternatives either added complexity without benefit (M\"{o}bius, rotors) or actively degraded retrieval quality (conceptors, Wasserstein). This is consistent with the interpretation that the embedding space geometry learned by the sentence transformers we tested is effectively Euclidean under cosine similarity, though we cannot rule out that different implementations of these alternatives might yield different results.
...
\textbf{What we discovered.} Perhaps the most unexpected finding is that \emph{embedding isotropy appears to govern operation efficacy}. Low-isotropy models (MiniLM: mean cosine 0.13) benefit from algebraic manipulation; high-isotropy models (E5: 0.81) are harmed. This variable, which we did not set out to investigate, offers a plausible explanation for the apparent contradiction between models where rotation helps and models where it hurts. We suspect this finding may have implications beyond retrieval, for other work that manipulates sentence-level representations, though confirming this requires further investigation.

\textbf{What remains open.} Rotation structurally displaces results toward target domains, and we validated this on scenarios in pharmacology (drug repurposing across 4 diseases) and law (cross-branch precedent analogy). But structural displacement is not the same as utility: whether the surfaced documents constitute \emph{genuinely useful} analogies for domain practitioners is an empirical question that can only be answered domain by domain, with expert evaluation, which we do not provide. This is an important next step --- and the reason we frame rotation as a mechanism that \emph{can be validated for utility}, not one whose utility is established. The domains most amenable to such validation are those where analogical reasoning is already an explicit professional practice: law (where analogy is a formal legal method), pharmacology (where 30\% of approved drugs have repositioning potential), and patent examination (where cross-industry prior art is a known challenge).

We see two main contributions in this work. First, an empirical map of the design space: we tested seventeen operations, identified embedding isotropy as a potentially governing variable, and documented what worked and what did not in our experiments. Second, the STEER adaptive stack: by composing isotropy correction, adaptive alpha, multi-vector fusion, and per-query LLM targets, we were able to reduce the nDCG degradation observed with naive rotation on contrastive models, achieving $+2\%$ to $+5\%$ nDCG on technical domains (TREC-COVID, SciFact) with minimal degradation elsewhere in our tests. The automatic operation router (F1$=0.91$ on TREC-COVID) provides initial evidence that the system may be able to decide \emph{when} and \emph{how} to steer without human intervention.

Our results suggest that query embeddings \emph{can} be algebraically transformed for retrieval purposes, and that the remaining challenge is \emph{how to make such transformation automatic, safe, and beneficial}. STEER provides initial progress toward this goal for biomedical and technical domains. Extension to other domains requires further experimentation and domain-expert evaluation.
...
All seventeen operations have now been empirically tested on BEIR benchmarks across six models. Two produced strong, reproducible results in our experiments (rotation and orthogonal subtraction). Three additional operations proved effective but are simpler variants of the same principles (SLERP $\equiv$ NLERP, vector addition $\approx$ rotation, geometric rotors $\equiv$ rotation). Scaled subtraction re-emerged as a practical improvement over full subtraction. Five operations proved neutral, and five failed outright. These results are consistent with the view that Euclidean operations on the unit hypersphere provide a natural basis for algebraic retrieval, though we cannot claim this is exhaustive.
